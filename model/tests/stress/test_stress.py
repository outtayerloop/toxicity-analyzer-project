...
class TestStress(BasePredictionEndpointTest):

    # Max expected response time in milliseconds (100 requests per minute SLA here)
    max_response_time_threshold = 600

    def test_prediction_endpoint_200_response_time_when_single_request(self):
        """
        Test if sending single valid request to the prediction endpoint
        does not take longer than the max accepted 200 OK response time threshold.
        """
        response = requests.post(self.prediction_model_endpoint_base_url,
                                 data=self.valid_json_content,
                                 headers=self.valid_headers)
        response_time = response.elapsed.total_seconds() * 1000
        assert response_time <= self.max_response_time_threshold

    def test_prediction_endpoint_stress_with_100_requests_per_minute_for_2_minutes(self):
        """
        Test if sending 100 valid requests per minute for 2 minutes to the prediction endpoint
        results in an average response delay inferior or equal to the max accepted one.
        """
        url = self.prediction_model_endpoint_base_url
        ...
        def do_post_to_prediction_endpoint(current_url: str) -> Response:
        ...
        for i in range(0, iteration_count):
            ...
            for j in range(0, iteration_length):  # we are going to send 2 request per second for 50 seconds
                ...
                with futures3.ThreadPoolExecutor(max_workers=request_count_per_second) as pool:
                    responses = list(pool.map(do_post_to_prediction_endpoint, [url] * request_count_per_second))
                ...
                if end_time_j < next_iteration_time_j:
                    ...
                    time.sleep(remaining_sleep_time_j)  # wait for next second
            ...
            if end_time_i < next_iteration_time_i and i < iteration_count - 1:
                ...
                time.sleep(remaining_sleep_time_i)  # wait for next minute
